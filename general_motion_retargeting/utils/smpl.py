...
def _expand_betas(betas, num_frames):
    betas = torch.tensor(betas).float().view(1, -1)
    return betas.expand(num_frames, -1)


def _zero_expression(body_model, num_frames):
    num_expr = getattr(body_model, "num_expression_coeffs", 10)
    return torch.zeros(num_frames, num_expr).float()


def _fit_betas_to_model(betas, body_model):
    num_betas = getattr(body_model, "num_betas", len(betas))
...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    smplx_output = body_model(
        betas=_expand_betas(smplx_data["betas"], num_frames), # (N, 16)
        ...
        expression=_zero_expression(body_model, num_frames),
...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    ...
    betas = _fit_betas_to_model(smpl_params_global['betas'][0].numpy(), body_model)
    smplx_data = {
